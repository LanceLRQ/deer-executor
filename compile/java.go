...
func (prov *JavaCompileProvider) Compile() (result bool, errmsg string) {
	result, errmsg = prov.shell(fmt.Sprintf(COMPILE_COMMAND_JAVA, prov.codeFilePath, path.Dir(prov.programFilePath)))
	if result {
		prov.isReady = true
	}
	return
...
func (prov *JavaCompileProvider) GetRunArgs() (args []string) {
	args = []string{
		"/usr/bin/java", "-client", "-Dfile.encoding=utf-8", "-Xmx768m",
